...
class MangaSearch extends Command {
        ...
        try {
            let mangaRequest = await axios({
                ...
                data: {
                    query: `query {
                          Page {
                            media(search: "${searchQuery}", type: MANGA) {
                              id
                              title {
                                romaji
                                english
            ...
            if (mangaRequest.data.data.Page.media.length === 0) {
            ...
            if (mangaRequest.data.data.Page.media.length === 1) {
                ...
                let pick = await new Menu(this.t('search.anime', {lngs: msg.lang}), this.t('menu.guide', {lngs: msg.lang}), mangaRequest.data.data.Page.media.map(a => {
                ...
                if (pick > -1) {
                    let manga = mangaRequest.data.data.Page.media[pick];
                    let characters = await this.loadCharacters(manga.id);
    ...
    buildResponse(msg, data, characters) {
        ...
        characterString = characterString.join(', ');let titleString = "";
            ...
            embed: {
                ...
                "fields": [
                    ...
                    },
                    {
                        ...
                        "value": `**${data.chapters > 0 ? data.chapters : `${this.t('generic.unknown', {lngs: msg.lang})}` }**`
                    },
                    {
